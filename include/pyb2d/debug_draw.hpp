...
namespace detail {
template <class CLS>
void GenericDrawPolygon(const b2Vec2 *vertices, int vertexCount,
                        b2HexColor color, void *context) {
  reinterpret_cast<CLS *>(context)->DrawPolygon(vertices, vertexCount, color);
}

template <class CLS>
void GenericDrawSolidPolygon(b2Transform transform, const b2Vec2 *vertices,
                             int vertexCount, float radius, b2HexColor color,
                             void *context) {
  reinterpret_cast<CLS *>(context)->DrawSolidPolygon(
      transform, vertices, vertexCount, radius, color);
}

template <class CLS>
void GenericDrawCircle(b2Vec2 center, float radius, b2HexColor color,
                       void *context) {
  reinterpret_cast<CLS *>(context)->DrawCircle(center, radius, color);
}

template <class CLS>
void GenericDrawSolidCircle(b2Transform transform, float radius,
                            b2HexColor color, void *context) {
  reinterpret_cast<CLS *>(context)->DrawSolidCircle(transform, radius, color);
}

template <class CLS>
void GenericDrawSolidCapsule(b2Vec2 p1, b2Vec2 p2, float radius,
                             b2HexColor color, void *context) {
...
template <class Derived> class DebugDraw : public b2DebugDraw {
public:
  DebugDraw() {
  ...
  void draw_polygon(const b2Vec2 *vertices, int vertexCount,
                    b2HexColor color) override {
  ...
  void draw_solid_polygon(b2Transform transform, const b2Vec2 *vertices,
                          ...
                          b2HexColor color) override {
    this->derived_cast()->draw_solid_polygon(transform, vertices, vertexCount,
                                             radius, color);
  }
  ...
  void draw_solid_cirlce(b2Transform transform, float radius,
                         b2HexColor color) override {
