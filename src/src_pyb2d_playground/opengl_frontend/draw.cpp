...
inline static RGBA8 MakeRGBA8(b2HexColor c, float alpha) {
...
b2Vec2 Camera::ConvertScreenToWorld(b2Vec2 ps) {
  ...
  b2Vec2 pw = {(1.0f - u) * lower.x + u * upper.x,
...
struct GLBackground {
  void Create(const char *dataDir) {
    ...
    glVertexAttribPointer(vertexAttribute, 2, GL_FLOAT, GL_FALSE, 0,
                          BUFFER_OFFSET(0));
  ...
  void Draw() {
    ...
    glUniform2f(m_resolutionUniform, (float)g_camera.m_width,
                (float)g_camera.m_height);
...
struct GLPoints {
  void Create(const char *dataDir) {
    const char *vs = "#version 330\n"
                     ...
                     "{\n"
                     "	f_color = v_color;\n"
                     "	gl_Position = projectionMatrix * vec4(v_position, "
                     "0.0f, 1.0f);\n"
                     "	gl_PointSize = v_size;\n"
                     "}\n";
    ...
    // Vertex buffer
    glBindBuffer(GL_ARRAY_BUFFER, m_vboId);
    glBufferData(GL_ARRAY_BUFFER, e_maxCount * sizeof(PointData), NULL,
                 GL_DYNAMIC_DRAW);

    glVertexAttribPointer(vertexAttribute, 2, GL_FLOAT, GL_FALSE,
                          sizeof(PointData),
                          (void *)offsetof(PointData, position));
    glVertexAttribPointer(sizeAttribute, 1, GL_FLOAT, GL_FALSE,
                          sizeof(PointData), (void *)offsetof(PointData, size));
    // color will get automatically expanded to floats in the shader
    glVertexAttribPointer(colorAttribute, 4, GL_UNSIGNED_BYTE, GL_TRUE,
                          sizeof(PointData), (void *)offsetof(PointData, rgba));
  ...
  void Flush() {
    ...
    while (count > 0) {
      ...
      glBufferSubData(GL_ARRAY_BUFFER, 0, batchCount * sizeof(PointData),
                      &m_points[base]);
...
struct GLLines {
  void Create(const char *dataDir) {
    ...
    // Vertex buffer
    glBindBuffer(GL_ARRAY_BUFFER, m_vboId);
    glBufferData(GL_ARRAY_BUFFER, e_maxCount * sizeof(VertexData), NULL,
                 GL_DYNAMIC_DRAW);
  ...
  void Flush() {
    ...
    while (count > 0) {
      int batchCount = b2MinInt(count, e_maxCount);
      glBufferSubData(GL_ARRAY_BUFFER, 0, batchCount * sizeof(VertexData),
                      &m_points[base]);
...
struct GLCircles {
  void Create(const char *dataDir) {
    m_programId =
        CreateProgramFromFiles((std::string(dataDir) + "/circle.vs").c_str(),
    ...
    glBufferData(GL_ARRAY_BUFFER, e_maxCount * sizeof(CircleData), NULL,
                 GL_DYNAMIC_DRAW);

    glVertexAttribPointer(positionInstance, 2, GL_FLOAT, GL_FALSE,
                          sizeof(CircleData),
                          (void *)offsetof(CircleData, position));
    glVertexAttribPointer(radiusInstance, 1, GL_FLOAT, GL_FALSE,
                          sizeof(CircleData),
                          (void *)offsetof(CircleData, radius));
    glVertexAttribPointer(colorInstance, 4, GL_UNSIGNED_BYTE, GL_TRUE,
                          sizeof(CircleData),
  ...
  void Flush() {
    ...
    while (count > 0) {
      ...
      glBufferSubData(GL_ARRAY_BUFFER, 0, batchCount * sizeof(CircleData),
                      &m_circles[base]);
...
// Draws SDF circles using quad instancing. Apparently instancing of quads can
// be slow on older GPUs.
...
struct GLSolidCircles {
  void Create(const char *dataDir) {
    ...
    // Circle buffer
    glBindBuffer(GL_ARRAY_BUFFER, m_vboIds[1]);
    glBufferData(GL_ARRAY_BUFFER, e_maxCount * sizeof(SolidCircleData), NULL,
                 GL_DYNAMIC_DRAW);

    glVertexAttribPointer(transformInstance, 4, GL_FLOAT, GL_FALSE,
                          sizeof(SolidCircleData),
  ...
  void Flush() {
    ...
    while (count > 0) {
      ...
      glBufferSubData(GL_ARRAY_BUFFER, 0, batchCount * sizeof(SolidCircleData),
                      &m_circles[base]);
      glDrawArraysInstanced(GL_TRIANGLES, 0, 6, batchCount);
...
struct GLSolidCapsules {
  void Create(const char *dataDir) {
    ...
    // Capsule buffer
    glBindBuffer(GL_ARRAY_BUFFER, m_vboIds[1]);
    glBufferData(GL_ARRAY_BUFFER, e_maxCount * sizeof(CapsuleData), NULL,
                 GL_DYNAMIC_DRAW);
    ...
    glVertexAttribPointer(lengthInstance, 1, GL_FLOAT, GL_FALSE,
                          sizeof(CapsuleData),
  ...
  void Flush() {
    ...
    while (count > 0) {
      ...
      glBufferSubData(GL_ARRAY_BUFFER, 0, batchCount * sizeof(CapsuleData),
                      &m_capsules[base]);
      glDrawArraysInstanced(GL_TRIANGLES, 0, 6, batchCount);
...
struct GLSolidPolygons {
  void Create(const char *dataDir) {
    ...
    // Polygon buffer
    glBindBuffer(GL_ARRAY_BUFFER, m_vboIds[1]);
    glBufferData(GL_ARRAY_BUFFER, e_maxCount * sizeof(PolygonData), NULL,
                 GL_DYNAMIC_DRAW);
    glVertexAttribPointer(instanceTransform, 4, GL_FLOAT, GL_FALSE,
                          sizeof(PolygonData),
                          (void *)offsetof(PolygonData, transform));
    glVertexAttribPointer(instancePoint12, 4, GL_FLOAT, GL_FALSE,
                          sizeof(PolygonData),
                          (void *)offsetof(PolygonData, p1));
    glVertexAttribPointer(instancePoint34, 4, GL_FLOAT, GL_FALSE,
                          sizeof(PolygonData),
                          (void *)offsetof(PolygonData, p3));
    glVertexAttribPointer(instancePoint56, 4, GL_FLOAT, GL_FALSE,
                          sizeof(PolygonData),
                          (void *)offsetof(PolygonData, p5));
    glVertexAttribPointer(instancePoint78, 4, GL_FLOAT, GL_FALSE,
                          sizeof(PolygonData),
                          (void *)offsetof(PolygonData, p7));
    glVertexAttribIPointer(instancePointCount, 1, GL_INT, sizeof(PolygonData),
                           (void *)offsetof(PolygonData, count));
    glVertexAttribPointer(instanceRadius, 1, GL_FLOAT, GL_FALSE,
                          sizeof(PolygonData),
                          (void *)offsetof(PolygonData, radius));
    // color will get automatically expanded to floats in the shader
    glVertexAttribPointer(instanceColor, 4, GL_UNSIGNED_BYTE, GL_TRUE,
                          sizeof(PolygonData),
  ...
  void AddPolygon(const b2Transform &transform, const b2Vec2 *points, int count,
  ...
  void Flush() {
    ...
    while (count > 0) {
      ...
      glBufferSubData(GL_ARRAY_BUFFER, 0, batchCount * sizeof(PolygonData),
                      &m_polygons[base]);
      glDrawArraysInstanced(GL_TRIANGLES, 0, 6, batchCount);
      CheckErrorGL();
...
void DrawPolygonFcn(const b2Vec2 *vertices, int vertexCount, b2HexColor color,
                    void *context) {
  static_cast<Draw *>(context)->DrawPolygon(vertices, vertexCount, color);
}

void DrawSolidPolygonFcn(b2Transform transform, const b2Vec2 *vertices,
...
void DrawCircleFcn(b2Vec2 center, float radius, b2HexColor color,
                   void *context) {
...
void DrawSolidCircleFcn(b2Transform transform, float radius, b2HexColor color,
                        void *context) {
  static_cast<Draw *>(context)->DrawSolidCircle(transform, b2Vec2_zero, radius,
                                                color);
}

void DrawSolidCapsuleFcn(b2Vec2 p1, b2Vec2 p2, float radius, b2HexColor color,
                         void *context) {
...
void Draw::DrawPolygon(const b2Vec2 *vertices, int vertexCount,
                       b2HexColor color) {
...
void Draw::DrawSolidPolygon(b2Transform transform, const b2Vec2 *vertices,
                            int vertexCount, float radius, b2HexColor color) {
...
void Draw::DrawSolidCircle(b2Transform transform, b2Vec2 center, float radius,
                           b2HexColor color) {
  transform.p = b2TransformPoint(transform, center);
  m_solidCircles->AddCircle(transform, radius, color);
}

void Draw::DrawSolidCapsule(b2Vec2 p1, b2Vec2 p2, float radius,
                            b2HexColor color) {
...
void Draw::DrawBackground() { m_background->Draw(); }
