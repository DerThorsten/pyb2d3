...
inline bool IsPowerOfTwo(int32_t x) { return (x != 0) && ((x & (x - 1)) == 0); }
...
static void MouseButtonCallback(GLFWwindow *window, int32_t button,
  ...
  if (g_draw.m_showUI) {
    ...
    if (ImGui::BeginTabBar("ControlTabs", ImGuiTabBarFlags_None)) {
      if (ImGui::BeginTabItem("Controls")) {
        ...
        if (ImGui::SliderInt("Workers", &s_settings.workerCount, 1,
                             maxWorkers)) {
          s_settings.workerCount =
              b2ClampInt(s_settings.workerCount, 1, maxWorkers);
          RestartSample();
        }
        ImGui::PopItemWidth();
...
int start_everything(const char *data_dir, nanobind::object sample_cls) {
  ...
  while (!glfwWindowShouldClose(g_mainWindow)) {
    ...
    ImGui_ImplGlfw_CursorPosCallback(g_mainWindow, cursorPosX / s_windowScale,
                                     cursorPosY / s_windowScale);
    ...
    if (g_draw.m_showUI) {
      snprintf(buffer, 128, "%.1f ms - step %d - camera (%g, %g, %g)",
               1000.0f * frameTime, s_sample->m_stepCount, g_camera.m_center.x,
