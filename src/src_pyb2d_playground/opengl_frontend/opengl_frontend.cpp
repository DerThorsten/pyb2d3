...
static void CreateUI(GLFWwindow *window, const char *glslVersion,
  ...
  if (file != NULL) {
    ...
    g_draw.m_smallFont = ImGui::GetIO().Fonts->AddFontFromFileTTF(
        fontPath.c_str(), 14.0f, &fontConfig);
    g_draw.m_mediumFont = ImGui::GetIO().Fonts->AddFontFromFileTTF(
        fontPath.c_str(), 40.0f, &fontConfig);
    g_draw.m_largeFont = ImGui::GetIO().Fonts->AddFontFromFileTTF(
        fontPath.c_str(), 64.0f, &fontConfig);
  } else {
...
GLFWwindow *OpenGlFrontend::main_window() { return m_mainWindow; }
...
void OpenGlFrontend::create_window() {
...
#ifdef __APPLE__
    glfwGetMonitorContentScale(primaryMonitor, &s_framebufferScale,
                               &s_framebufferScale);
#else
  ...
  if (fullscreen) {
    m_mainWindow =
        glfwCreateWindow(int(1920 * s_windowScale), int(1080 * s_windowScale),
                         title, glfwGetPrimaryMonitor(), nullptr);
  } else {
    m_mainWindow = glfwCreateWindow(int(g_camera.m_width * s_windowScale),
...
#ifdef __APPLE__
  glfwGetWindowContentScale(m_mainWindow, &s_framebufferScale,
                            &s_framebufferScale);
#else
