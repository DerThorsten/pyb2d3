...
class PolygonFontAlphabet(object):
    ...
    def _init_char(self, char):
        ...
        mid = (
            self.img_size[0] // 2 - self.font_size // 2,
            self.img_size[1] // 2 - self.font_size // 2,
        )
        ...
        for contour, holes in cleaned:
            ...
            polygon_simple = shapely.simplify(
                polygon, tolerance=0, preserve_topology=True
            )
        ...
        if self.debug:
            print(
                f"char: {char}, # of triangles: {len(triangulated_all)} #contours: {len(contours)}"
            )
            # plot triangulated
            fig, ax = plt.subplots()
            ax.imshow(img_array)
