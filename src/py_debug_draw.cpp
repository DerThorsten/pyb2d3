...
void PyDrawPolygon(const b2Vec2 *vertices, int vertexCount, b2HexColor color,
                   void *context) {
  PyDebugDraw *draw = reinterpret_cast<PyDebugDraw *>(context);
  float *data = const_cast<float *>(reinterpret_cast<const float *>(vertices));
  ArrayVec2 points(data, {static_cast<std::size_t>(vertexCount),
                          static_cast<std::size_t>(2)});
  draw->m_py_class.attr("draw_polygon")(points, static_cast<int>(color));
}

/// Draw a solid closed polygon provided in CCW order.
void PyDrawSolidPolygon(b2Transform transform, const b2Vec2 *vertices,
  ...
  ArrayVec2 points(data, {static_cast<std::size_t>(vertexCount),
                          static_cast<std::size_t>(2)});
  draw->m_py_class.attr("draw_solid_polygon")(transform, points, radius,
                                              static_cast<int>(color));
}

/// Draw a circle.
void PyDrawCircle(b2Vec2 center, float radius, b2HexColor color,
                  void *context) {
  PyDebugDraw *draw = (PyDebugDraw *)context;
  draw->m_py_class.attr("draw_circle")(center.x, center.y, radius,
                                       static_cast<int>(color));
}

/// Draw a solid circle.
void PyDrawSolidCircle(b2Transform transform, float radius, b2HexColor color,
                       void *context) {
  PyDebugDraw *draw = (PyDebugDraw *)context;
  draw->m_py_class.attr("draw_solid_circle")(transform, radius,
                                             static_cast<int>(color));
}

/// Draw a solid capsule.
void PyDrawSolidCapsule(b2Vec2 p1, b2Vec2 p2, float radius, b2HexColor color,
                        void *context) {
  PyDebugDraw *draw = (PyDebugDraw *)context;
  draw->m_py_class.attr("draw_solid_capsule")(p1, p2, radius,
                                              static_cast<int>(color));
}
...
PyDebugDraw::PyDebugDraw(py::handle py_class)
    : b2DebugDraw(b2DefaultDebugDraw()), m_py_class(py_class) {
