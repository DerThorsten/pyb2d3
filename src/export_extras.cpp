...
{
    CapsuleBuilderWithTransform(
        CanvasWorldTransform const& transform,
        std::size_t max_circle_segments=2
    )
    ...
    std::size_t build(b2Vec2 const& center1, b2Vec2 const& center2, float radius)
    {
...
void export_extras(nb::module_& m)
{
    ...
    nb::class_<CapsuleBuilderWithTransform>(m, "CapsuleBuilderWithTransform")
        .def(nb::init<const CanvasWorldTransform&, std::size_t>(),
            nb::keep_alive<0, 1>(),  // keep the transform alive
            nb::arg("transform"),
            nb::arg("max_circle_segments")
        )
        .def("get_vertices_buffer", [](CapsuleBuilderWithTransform& self) {
            // convert the vector of int to a numpy array
            return ArrayVec2Int(
                    reinterpret_cast<int*>(self.vertices.data()),
                    {
                        std::size_t(self.max_circle_segments*2 + 4), std::size_t(2)
                    }  // shape
                );
        },
